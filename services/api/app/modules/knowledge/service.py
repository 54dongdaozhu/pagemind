...
def _kp_status_key(user_id: str, kp_text: str) -> str:
    return f"study:status:u:{user_id}:kp:{stable_hash(kp_text)}"


def _stats_key(user_id: str) -> str:
    return f"study:stats:u:{user_id}"
...
def record_click(user_id: str, kp_text: str, kp_type: str):
    ...
    set_json(_kp_status_key(user_id, kp_text), {"status": new_status, "click_count": new_count}, _KP_STATUS_TTL)
    delete_key(_stats_key(user_id))
    db_log.log_event(
...
def mark_known(user_id: str, kp_text: str, kp_type: str):
    ...
    set_json(_kp_status_key(user_id, kp_text), {"status": STATUS_KNOWN, "click_count": record.click_count}, _KP_STATUS_TTL)
    delete_key(_stats_key(user_id))
    db_log.log_event(
        entity_type="study_record",
        entity_id=record.record_id,
...
def unmark_known(user_id: str, kp_text: str):
    ...
    set_json(_kp_status_key(user_id, kp_text), {"status": new_status, "click_count": record.click_count}, _KP_STATUS_TTL)
    delete_key(_stats_key(user_id))
    db_log.log_event(
        entity_type="study_record",
        entity_id=record.record_id,
...
def get_status_batch(user_id: str, kp_texts: list[str]):
    ...
    keys = [_kp_status_key(user_id, t) for t in kp_texts]
    cached_values = get_json_many(keys)

    result: dict[str, dict] = {}
    missing_texts: list[str] = []
    for text, val in zip(kp_texts, cached_values):
        if val is not None:
            result[text] = val
        else:
    ...
    if missing_texts:
        with get_db() as db:
            rows = db.execute(
                select(StudyRecord).where(
                    StudyRecord.user_id == user_id,
                    StudyRecord.kp_text.in_(missing_texts),
                )
            ).scalars().all()

        new_cache: dict[str, dict] = {}
        for row in rows:
            val = {"status": row.status, "click_count": row.click_count}
            result[row.kp_text] = val
            new_cache[_kp_status_key(user_id, row.kp_text)] = val
        if new_cache:
            set_json_many(new_cache, _KP_STATUS_TTL)
