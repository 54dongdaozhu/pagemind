...
def _search_document_chunks(user_id: str, doc_id: str, query: str, top_k: int = 4):
    return retrieve_relevant_chunks(user_id=user_id, doc_id=doc_id, question=query, top_k=top_k)


def _read_document_summary(user_id: str, doc_id: str) -> str:
...
def _get_knowledge_status_batch(user_id: str, kp_texts: list[str]):
    return get_status_batch(user_id, kp_texts)


def _record_knowledge_click(user_id: str, kp_text: str, kp_type: str):
    return record_click(user_id=user_id, kp_text=kp_text, kp_type=kp_type)


def _mark_knowledge_known(user_id: str, kp_text: str, kp_type: str):
    return mark_known(user_id=user_id, kp_text=kp_text, kp_type=kp_type)


def _unmark_knowledge_known(user_id: str, kp_text: str):
    return unmark_known(user_id=user_id, kp_text=kp_text)


def _get_learning_stats(user_id: str):
...
def _extract_document_structure(text: str):
    messages = [
...
def _map_knowledge_relations(context: str, knowledge_points: list[str] | None = None):
    kp_text = (
        "\n".join(f"- {item}" for item in knowledge_points)
        if knowledge_points
        else "未提供，请从上下文中自行识别关键知识点。"
    )
    messages = [
...
TOOL_REGISTRY: dict[str, AgentTool] = {
    "read_document_summary": AgentTool(
        ...
        args_schema={"doc_id": {"type": "string", "description": "当前文档 ID。"}},
        return_schema={"summary": {"type": "string", "description": "文档摘要，可能为空字符串。"}},
        when_to_use="当 agent 需要先了解整篇文档背景、回答总结类问题或为检索结果补充全局上下文时使用。",
        constraints=["只能读取已通过 RAG 索引保存过摘要的文档。", "不负责生成最终回答。"],
        function=_read_document_summary,
    ),
    "search_document_chunks": AgentTool(
        ...
        return_schema={"sources": {"type": "array", "description": "相关片段列表，每项包含 chunk_index、content、score、retrieval_method。"}},
    ...
    "extract_knowledge_from_chunk": AgentTool(
        ...
        args_schema={"text": {"type": "string", "description": "待识别的文档片段原文。"}},
        return_schema={"knowledge_points": {"type": "array", "description": "知识点列表，每项包含 text、type、explanation、importance。"}},
        when_to_use="文档上传、重新识别高亮、或 agent 需要从某段上下文中抽取学习对象时使用。",
        constraints=["text 字段必须能在原文中找到。", "不按固定数量返回，空数组是有效结果。"],
        function=_extract_knowledge_from_chunk,
    ),
    "get_knowledge_status_batch": AgentTool(
        name="get_knowledge_status_batch",
        description="批量读取知识点的学习状态和点击次数。",
        args_schema={"kp_texts": {"type": "array", "description": "知识点文本列表。"}},
        return_schema={"items": {"type": "array", "description": "状态列表，每项包含 kp_text、status、click_count。"}},
        when_to_use="需要判断用户对哪些知识点已学习、学习中或已掌握时使用。",
        constraints=["只返回已有学习记录的知识点；没有记录代表未知状态。"],
        function=_get_knowledge_status_batch,
    ),
    "record_knowledge_click": AgentTool(
        ...
        function=_record_knowledge_click,
    ),
    "mark_knowledge_known": AgentTool(
        ...
        return_schema={"kp_text": {"type": "string"}, "status": {"type": "string", "description": "固定为 known。"}},
        when_to_use="用户明确表示已掌握某个知识点时使用。",
        constraints=["这是有副作用的写入工具，必须由明确用户行为触发。"],
        function=_mark_knowledge_known,
    ),
    "unmark_knowledge_known": AgentTool(
        name="unmark_knowledge_known",
        description="取消某个知识点的已掌握状态，并根据历史点击次数恢复为 unknown 或 learning。",
        args_schema={"kp_text": {"type": "string", "description": "知识点文本。"}},
        return_schema={"kp_text": {"type": "string"}, "status": {"type": "string", "description": "恢复后的状态。"}},
        when_to_use="用户明确取消掌握标记时使用。",
        constraints=["这是有副作用的写入工具，必须由明确用户行为触发。"],
        function=_unmark_knowledge_known,
    ),
    "get_learning_stats": AgentTool(
        ...
        constraints=["只反映数据库中已有记录。"],
        function=_get_learning_stats,
    ),
    ...
    "map_knowledge_relations": AgentTool(
        ...
        return_schema={"relations": {"type": "array", "description": "关系列表，包含 source、target、relation、reason。"}},
        when_to_use="用户想看概念关系、知识图谱、区别联系、前置依赖或学习路径时使用。",
        constraints=["关系必须能从上下文合理推出。", "不要编造文档没有支持的强关系。"],
        function=_map_knowledge_relations,
    ),
...
def list_tools() -> list[dict[str, Any]]:
    return [t.public_dict() for t in TOOL_REGISTRY.values()]
